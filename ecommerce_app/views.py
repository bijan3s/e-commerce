...
def show_cart(request):
    ...
    if request.method == 'POST':
        if request.POST.get('submit') == 'تغییر':
...
def checkout(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            o = Order(
                name = cleaned_data.get('نام'),
                email = cleaned_data.get('ایمیل'),
                postal_code = cleaned_data.get('کد_پستی'),
                address = cleaned_data.get('آدرس'),
            )
            o.save()
            ...
            messages.add_message(request, messages.INFO, ':) سفارش انجام شد')
